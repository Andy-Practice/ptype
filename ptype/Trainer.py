...
class Trainer:
    ...
    def conjugate_gradient(self, w, J=10, gtol=1e-5):
        ...
        while gnorm > gtol and j < J:
            ...
            if res[0] is None:
                ...
                alpha = res[0]
                w = w + alpha * d[j] # New probabilities = old + step-size (alpha) *dirction.
 
                g.append(self.g_cols(w)) # add the new gradient vector - gradient that minimises . 
    ...
    def f_col(self, all_probs, i, column_name, y_i):
        ...
        # q is un-normalised postetior distribution for that type. 
        temp = normalize_log_probs(q)[y_i] # normalised for y_i - correct column type. 
        if temp == 0:
            return 800.0
        else:
            return -np.log(temp)
    ...
    def g_col_marginals(self, all_probs, i, col_name, y_i):
        ...
        logP = np.array([all_probs[x] for x in xs]) # Log=probability - array of all probabilities for unique_values in the data. 
        ...
        for t, ty in enumerate(self.machines.types):
            ...
            if t == 1:
                ...
                marginals = {
                    x_i: machine.run_forward_backward(str(x_i)) # x_i is marginal probability or data/that data is generated by that machine. 
                    if p_x_i[t + 2] != LOG_EPS
    ...
    def g_cols(self, w_j_z):
        ...
        return q_total # The gradient vector - obtained using forward backward algorithm to get marginal probability ...
